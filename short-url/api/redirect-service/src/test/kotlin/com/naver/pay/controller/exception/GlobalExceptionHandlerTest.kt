...
class TestController {
    ...
    fun throwExpiredLinkException() {
        throw ExpiredLinkException("Test Gone")
    }
...
class GlobalExceptionHandlerTest: BehaviorSpec() {
    ...
    init {
        Given("API 요청 처리 중 예외가 발생했을 때") {
            ...
            When("ExpiredLinkException 발생하면") {
                val result = mockMvc.get("/test/gone")

                Then("410 Gone과 함께 ErrorResponse를 반환한다") {
                    result.andExpect {
                        status { isGone() }
                        content {
                            contentType("application/json")
                            jsonPath("$.status") { value(HttpStatus.GONE.name) }
                            jsonPath("$.error") { value(ExpiredLinkException::class.java.simpleName) }
                            jsonPath("$.message") { value("The link 'Test Gone' has expired.") }
                            jsonPath("$.path") { value("/test/gone") }
                        }
                    }
                }
            }
