...
Gateway 요청 흐름은 정책 일원화와 요청 처리 안정성을 핵심으로 설계되었습니다. 각 단계는 서로 의존하며, 전체 구조는 “앞단에서 최대한 차단하고, 뒤로 갈수록 최소한의 검증만 수행한다”는 원칙을 기반으로 구성됩니다.
...
- 무분별한 요청을 차단하고, 테넌트 별로 정책을 적용하기 위한 기초 데이터로 활용합니다.
...
**한계:**
  ...
  - Redis 장애 시 전체 인증 시스템이 마비될 수 있는 의존성이 생깁니다.
  - 유저 서비스의 부재로 개별 자원에 대한 권한 관리는 제공되지 않습니다.

서비스 대부분의 트래픽인 리다이렉트는 인증/인가가 발생하지 않습니다. 따라 redis IO가 발생하지 않습니다. 

그 외의 트래픽은 수가 적고, 보안 통제 및 세션 관리가 중요하다 생각하여 Stateful한 Api Key 기반 인증/인가를 채택하였습니다.

#### 3. 장애 격리 및 회복 탄력성 (Resilience)

시스템의 안정성을 위해 `spring-cloud-circuit-breaker`를 도입하여, 특정 서비스의 장애가 게이트웨이 전체 리소스(Thread Pool) 고갈로 이어지는 것을 방지합니다. 각 API의 역할과 부하 특성에 맞춰 임계치를 세밀하게 조정했습니다.
...
- 멱등성이 보장되므로 실패율을 50%까지 관대하게 허용하여, 일시적인 데드락이나 지연 시 즉시 차단하기보다 클라이언트의 재시도를 유도합니다.
